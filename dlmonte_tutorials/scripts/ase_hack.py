...
def read_single_dlm_image(f, levcfg, imcon, natoms, is_trajectory, symbols=None, verbose=False):
    ...
    while line and (a < counter):
        ...
        if not symbols:
            # A quick hack to handle pseudo atoms as Helium
            # NB probably breaks some ASE style guides, revert to assertion if not
            # assert symbol in chemical_symbols, 'Line reading error, line is {0:}, symbol is {1}'.format(line, symbol)
            if not symbol in chemical_symbols:
                sym.append('He')
            else:
                sym.append(symbol)
